...
class CallRoutingService
{
     ...
     * Core routing decision: forward to tradie or hand to AI.
     *
     * Tradie is identified by the called number (their Twilio business number).
     * If available, forward. If not, hand to AI.
     */
    public function routeIncomingCall(IncomingCallData $data): CallRoutingResult
    {
        // Find tradie by their business number
        $tradie = $this->tradies->findByBusinessNumber($data->calledNumber);
        ...
        // Atomically claim the tradie if available — pass tradie ID not tenant ID
        $claimedTradie = $this->availability->claimAvailableTradie($tradie->id);

        if ($claimedTradie) {
            return CallRoutingResult::forwardTo($claimedTradie->id, $claimedTradie->personal_phone);
        }
    ...
    /**
     * Build TwiML to forward the call to tradie's personal phone.
     */
    public function buildForwardTwiml(Call $call, string $personalPhone, string $appBaseUrl): TwimlResponse
    {
    ...
    /**
     * Build TwiML to hand control to Retell AI.
     */
    public function buildAIHandoffTwiml(string $retellWebsocketUrl): TwimlResponse
    {
        return $this->provider->buildAIHandoffResponse($retellWebsocketUrl);
